...
## Model Behavior
I managed to quite consistently defuse first two modules. Unfortunately i wasn't able to crack Simon module. I think the concept of the 'round' was quite misleading for the LLM and it often outputted the correct command according to its concept of a round. In my opinion, that module is very hard without editing the manual in some way. 

##  Common Challenges
I struggled a bit with the setup, because of the constant errors, which turned out to be related to the fact, that the model did not conform to the required style of the prompt (for example it passed a dictionary instead of the string as a tool argument). When i got it working it couldn't defuse more then one module. It turned out that agents cache the tool output by default and after turning it off, the agent started to cooperate well.
